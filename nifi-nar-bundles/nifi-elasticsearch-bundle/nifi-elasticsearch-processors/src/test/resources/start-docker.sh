...
docker run --name SQUID_NIFI -d --restart=always --publish $4:3228 --volume $6/squid.conf:/etc/squid/squid.conf  --volume /srv/docker/squid/cache:/var/spool/squid --network=es_squid sameersbn/squid:3.5.27-2
docker run --name SQUID_NIFI_AUTH -d --restart=always --publish $5:3328 --volume $6/squid-auth.conf:/etc/squid/squid.conf  --volume /srv/docker/squid/cache:/var/spool/squid --network=es_squid sameersbn/squid:3.5.27-2
